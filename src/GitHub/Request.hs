  ...
  where
    ...
    extraQueryItems = case r of
      PagedQuery _ _ (FetchPage pp) -> catMaybes [
          (\page -> ("page", [QE (BS.toStrict $ toLazyByteString $ intDec page)])) <$> pageParamsPage pp
          , (\perPage -> ("per_page", [QE (BS.toStrict $ toLazyByteString $ intDec perPage)])) <$> pageParamsPerPage pp
          ]
      _ -> []
